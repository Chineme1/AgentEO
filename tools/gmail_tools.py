...
class GetUnreadEmailsTool(GmailToolBase):
    ...
    def _run(self, limit: Optional[int] = 5) -> List[Tuple[str, str, str, str, Dict]]:
        ...
        try:
            ...
            email_ids = data[0].split()  #This is the total number of EMAILS
...
#Modified Version
class TotalEmailTool(GmailToolBase):
    """Tool to get the total number of emails in the Gmail inbox."""
    name: str = "total_email"
    description: str = "Gets the total number of emails in the Gmail inbox"
    
    def _run(self) -> int:
        """
        Returns the total number of emails in the Gmail INBOX.
        """
        mail = self._connect()
        try:
            # Select the inbox
            result, data = mail.select("INBOX") #All Mail
            if result != "OK":
                return f"Error selecting INBOX: {result}"
            
            # Search for all emails in the inbox
            result, data = mail.search(None, "ALL")
            if result != "OK":
                return f"Error searching INBOX: {result}"
            ...
            return f"Error getting total email count: {str(e)}"
        finally:
            self._disconnect(mail)